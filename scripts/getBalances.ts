...
async function getData() {
    ...
    try{
        ...
        console.log(formatUnits(BigNumber.from(balances[4591]), 9).replace(/\./g, ','))
        Object.keys(addresses).forEach((address, index) => {
            ...
            if(sheet.getCell(sheetindex, 0).value == address){
                if(address == "0x1c8894aa87cab3108d034cb1e546c9b1e64924e1"){
                    sheet.getCell(sheetindex, 1).value = 0;
                }
                else if(address == "0xEC98a19021f29ce424cB28E1c41f293434518691"){
                    sheet.getCell(sheetindex, 1).value = 2476983880785
                }
                else{
